...
export class ClassComponent extends React.Component {
  constructor(props) {
    ...
    this.RefGuess = React.createRef();
    this.RefPlayAgain = React.createRef();
  }
  ...
  handleSubmit = e => {
    ...
    this.setState(state => {
      ...
      const playingAgain = this.RefPlayAgain.current;
      playingAgain.style.display = 'block';

      const guess = this.RefGuess.current;
    ...
    return (
      <div className={style.game}>
        ...
        <form className={style.form} onSubmit={this.handleSubmit}>
          ...
          <button className={style.btn} id='guess' ref={this.RefGuess}>
            Угадать
          </button>
          <button className={`${style.btn} ${style.hiddenBtn}`}
            ref={this.RefPlayAgain} onClick={this.playAgain}>Сыграть еще
          </button>
        </form>
      </div>
    );
